...
export default function AdminRegister() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post("http://localhost:5000/api/admin/register", form, { withCredentials: true });
      alert("Admin Registered");
      navigate("/admin/login");
    } catch (err) {
      alert(err.response?.data?.msg || "Error");
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-[#003848] to-[#8b1d41] p-4">
      <div className="bg-white shadow-2xl rounded-2xl w-full max-w-md p-8">
        <h2 className="text-3xl font-bold text-center text-[#003848] mb-6">Admin Register</h2>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <input type="text" placeholder="Name" onChange={(e) => setForm({ ...form, name: e.target.value })} required
            className="w-full px-4 py-3 border rounded focus:ring-2 focus:ring-[#8b1d41]" />
          <input type="email" placeholder="Email" onChange={(e) => setForm({ ...form, email: e.target.value })} required
            className="w-full px-4 py-3 border rounded focus:ring-2 focus:ring-[#8b1d41]" />
          <input type="password" placeholder="Password" onChange={(e) => setForm({ ...form, password: e.target.value })} required
            className="w-full px-4 py-3 border rounded focus:ring-2 focus:ring-[#8b1d41]" />
          <button type="submit" disabled={loading}
            className={`w-full py-3 text-white font-semibold rounded mt-2 ${loading ? "bg-gray-500 cursor-not-allowed" : "bg-[#003848] hover:bg-[#002233]"}`}>
            {loading ? "Registering..." : "Register"}
          </button>
        </form>
        <div className="mt-4 text-center text-gray-500 text-sm">
          Already an admin? <span className="text-[#8b1d41] cursor-pointer" onClick={() => navigate("/admin/login")}>Login</span>
        </div>
      </div>
    </div>
  );
